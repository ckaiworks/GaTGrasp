#!/usr/bin/env python3
"""Deterministic single-output Stage-2 with contact-aligned GS.

Control-variable pair for the clean RGB+Action+Touch verifier.  This model
adds only the left/right contact GS residual; the closing corridor is never
read.  Training uses one success logit and weighted BCE only.
"""
...
NOGS = load_module("stage2_clean_singlehead_nogs_control", NOGS_PATH)
BASE = NOGS.BASE
SOURCE = NOGS.SOURCE
TouchLRCDataset = NOGS.TouchLRCDataset


class CleanSingleHeadStage2WithContactGS(NOGS.CleanSingleHeadStage2):
    """RGB+Action+Touch predictor with a left/right contact-GS residual."""

    def __init__(self):
        # Construct the complete noGS model first.  With the same seed this
        # makes every common parameter bit-identical to the noGS control.
        super().__init__()
        self.endpoint_encoder = BASE.PatchEncoder()
        self.side_fusion = nn.Sequential(
            nn.Linear(128 + 64 + 64, 128),
            nn.ReLU(inplace=True),
            nn.Dropout(0.25),
        )
        self.gs_touch_relation = nn.Sequential(
            nn.Linear(128 * 4, 128),
            nn.ReLU(inplace=True),
            nn.Dropout(0.25),
        )
        self.gs_delta_head = nn.Sequential(
            nn.Linear(128, 64),
            nn.ReLU(inplace=True),
            nn.Dropout(0.15),
            nn.Linear(64, 1),
        )
        # The withGS model begins at the exact noGS function.  The first
        # optimizer step opens the residual, then gradients reach all GS
        # modules from the following step onward.
        nn.init.zeros_(self.gs_delta_head[-1].weight)
        nn.init.zeros_(self.gs_delta_head[-1].bias)
    ...
    def forward(
        ...
        # Kept only for the common dataset/API.  Corridor is not a modality
        # of this experiment.
        del corridor, corridor_mask
...
def main():
    ...
    if args.smoke:
        ...
        BASE.set_seed(args.seed)
        reference = NOGS.CleanSingleHeadStage2().to(device)
        BASE.set_seed(args.seed)
        model = CleanSingleHeadStage2WithContactGS().to(device)
        common_differences = []
        ...
        with torch.inference_mode():
            initial_reference = NOGS.model_forward(reference, batch)
        ...
        with torch.inference_mode():
            ...
            corridor_invariance = float(torch.max(torch.abs(
                output - model_forward(model, changed_corridor)
            )).item())
        if gs_sensitivity <= 0.0:
            raise RuntimeError(f"CONTACT_GS_NOT_READ={gs_sensitivity}")
        if corridor_invariance != 0.0:
            raise RuntimeError(f"CORRIDOR_IS_READ={corridor_invariance}")
        print("CLEAN_SINGLEHEAD_WITHGS_SMOKE_PASS", {
            ...
            "corridor_invariance_max_diff": corridor_invariance,
    ...
    contract = {
        "experiment": (
            "stage2_clean_singlehead_rgb_action_touch_contactgs_"
            "weighted_bce_v1"
        ),
        "control_pair": (
            "stage2_clean_singlehead_rgb_action_touch_weighted_bce_v1"
        ),
        "training_initialization": "fresh end-to-end",
        "output": "one success logit",
        "inputs_used": [
            "RGB", "Action13", "bilateral real Touch",
            "left/right contact-aligned GS",
        ],
        "inputs_not_used": ["closing corridor reconstructed surface"],

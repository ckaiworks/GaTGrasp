...
COMMON = Path(__file__).with_name('stage1_common.py')
spec = importlib.util.spec_from_file_location('stage1_common', COMMON)
...
def evaluate(model, loader, device, criterion):
    ...
    for batch in loader:
        b = move(batch, device)
        logit = model(b['rgb'], b['action_geom'], b['left'], b['right'], b['corridor'], b['left_mask'], b['right_mask'], b['corridor_mask'])
